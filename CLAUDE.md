...
- **Finished-result scan rows (v240):** the AI Picks finished-game fold no
  ...
  Each market is now its own compact result row with a green check for a win,
  red miss for a loss, gold equals for a push and a neutral dash when no pick
  was logged. This is presentation-only; saved picks, grading and the v239
  evaluation cohort are unchanged.
...
## 💾 Storage & lifecycle — measured, not guessed (Sep 2026)

The owner asked whether two apps in one repo is "too much for GitHub and
everything". **It is nowhere near any hosting limit.** The one number that IS
...
- ⚠️ **`sportshub:aitally` is the one key that grows without bound.** It used
  ...
  not urgent; if it ever needs capping, cap by DATE, never by count, or the
